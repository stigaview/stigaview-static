...
def main() -> int:
    ...
    for current_zip in zip_files:
        ...
        with zipfile.ZipFile(full_zip_path, 'r') as z:
            product_regex = r'U_(?P<product>.+)_V(?P<version>\d+)R(?P<release>\d+)_STIG\.zip'
                ...
                if file_info.filename.endswith('.xml'):
                    with z.open(file_info) as file:
                        ...
                        if not output_path.exists():
                            ...
                            print(f"File {str(output_path)} already exists for {short_name}.", file=sys.stderr)
                        product_toml_path = root / "products" / short_name / 'product.toml'
